<template>
    <div class="grid grid-cols-12 gap-8 mx-[8%] my-16">
        ...
        <div class="col-start-5 col-span-4 flex justify-center items-end">
            <p>実験No:　</p>
            <div v-if="siteType === '' && subjectID === ''">
                <h1 class="text-4xl text-primary">????</h1>
            </div>
            <div v-if="siteType === '' && subjectID !== ''">
                <h1 class="text-4xl text-primary">{{ "?" + subjectID }}</h1>
            </div>
            <div v-if="siteType !== '' && subjectID === ''">
                <h1 class="text-4xl text-primary">{{ siteType + "???" }}</h1>
            </div>
            <div v-if="siteType !== '' && subjectID !== ''">
                <h1 class="text-4xl text-primary">{{ siteType + subjectID }}</h1>
            </div>
        </div>
        ...
        <div class="grid grid-cols-subgrid gap-8 col-span-12 py-8 flex items-center">
            <div class="col-start-5 col-span-4">
                <label class="input input-bordered flex items-center gap-2">
                    ...
                    <input type="text" v-model="subjectID" class="grow text-xl" placeholder="被験者ID" />
                </label>
            </div>
            ...
            <div class="col-start-5 col-span-4">
                <label class="input input-bordered flex items-center gap-2">
                    ...
                    <input type="text" v-model="siteType" class="grow text-xl" placeholder="実験種別" />
                </label>
            </div>

            <div class="col-start-6 col-span-2">
                <NuxtLink to="/top"><button class="btn btn-primary text-lg w-full">実験を開始する</button></NuxtLink>
            </div>
        </div>
    </div>
</template>
